	...
	for (sprite in sprites_list) {
		if (!(typeof sprites_list[sprite] === 'undefined') && !(typeof sprites_list[sprites_list[sprite].ai.targets] === 'undefined')) {
			if (sprites_list[sprite].use_ai == true) {
				...
				// console.log(xpos_altered + sprite)
				
				if ( xpos_altered > sprites_list[sprite].x_pos ) {
							sprites_list[sprite].x_speed += Math.floor(calculate_distance(sprite,target_sprite));
						} else {
							
							sprites_list[sprite].x_speed += 0 - Math.floor(calculate_distance(sprite,target_sprite));
						}
						if ( ypos_altered > sprites_list[sprite].y_pos ) {
							sprites_list[sprite].y_speed += Math.floor(calculate_distance(sprite,target_sprite));
						} else {
							
							sprites_list[sprite].y_speed += 0 - Math.floor(calculate_distance(sprite,target_sprite));
						}
				...
				if (sprites_list[sprite].ai.type == "goblin") {
					...
					if (sprites_list[sprite].ai.targets.length > 0) {
						...
						if ( sprites_list[sprite].x_pos > xpos_altered) {
							...
							sprites_list[sprite].x_speed -= Math.floor((Math.random() * 50 * 2) + -50);
						}
						if ( sprites_list[sprite].x_pos < xpos_altered) {
						...
						if ( sprites_list[sprite].y_pos > ypos_altered) {
							...
							sprites_list[sprite].y_speed -= Math.floor((Math.random() * 50 * 2) + -50);
						}
						if ( sprites_list[sprite].y_pos < ypos_altered) {
				...
				if (sprites_list[sprite].y_speed > 20) {sprites_list[sprite].y_speed = 20}
				if (sprites_list[sprite].y_speed < -20) {sprites_list[sprite].y_speed = -20}
				if (sprites_list[sprite].x_speed > 20) {sprites_list[sprite].x_speed = 20}
				if (sprites_list[sprite].x_speed < -20) {sprites_list[sprite].x_speed = -20}
...
//function objective_completed(objective_name) {
//	delete(objectives[objective_name]);
// }
	...
	for (sprite in sprites_list) {
		...
		if (!(typeof sprites_list[sprite] === 'undefined') && !(typeof sprites_list[sprites_list[sprite].ai.targets] === 'undefined')) {
			if (sprites_list[sprite].use_ai == true) {
				...
				/*
				if ( xpos_altered > sprites_list[sprite].x_pos ) {
							sprites_list[sprite].x_speed += calculate_distance(sprite,target_sprite)/ 5;
						} else {
							
							sprites_list[sprite].x_speed += 0 - calculate_distance(sprite,target_sprite)/ 5;
						}
						if ( ypos_altered > sprites_list[sprite].y_pos ) {
							sprites_list[sprite].y_speed += calculate_distance(sprite,target_sprite)/ 5;
						} else {
							
							sprites_list[sprite].y_speed += 0 - calculate_distance(sprite,target_sprite)/ 5;
						}
				*/
